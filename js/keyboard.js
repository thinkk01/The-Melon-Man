...
game.moveLeft = function () {
  ...
  game.player.moveLeftInterval = setInterval(function () {
    for (var i = 1; i < 120; i++) {
      setTimeout(function () {
        if (game.player.isInAir && !game.player.jumpInterrupted) {
          game.player.x -= 0.2;
        } else {
          game.player.x -= 0.1;
        }
        game.requestRedraw();
        if (!game.checkCollisions() && !game.player.isInAir) {
          game.player.jump("fall");
        }
      }, 3 * i);
    }
    if (game.player.animations2.frameX >= game.player.animations2.left.length) {
      game.player.animations2.frameX = 0;
    }
    game.player.animationFrameNumber++;
  }, 120);
};
game.moveLeft1 = function () {
  game.player.direction = "left";
  game.clearMoveIntervals();
  game.player.moveLeftInterval = setInterval(function () {
    for (var i = 1; i < 120; i++) {
